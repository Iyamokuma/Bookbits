...
import { asyncRoute, unauthorized, badRequest, notFound } from './helpers.js';
...
async function saveCover(file) {
  ...
  const name = `cover_${crypto.randomBytes(8).toString('hex')}.${ALLOWED_MIME[file.mimetype]}`;
  await fs.writeFile(path.join(UPLOAD_DIR, name), file.buffer);
  return `uploads/covers/${name}`;
}

async function deleteCover(relative) {
  if (!relative?.startsWith('uploads/covers/')) return;
  await fs.unlink(path.join(process.cwd(), 'public', relative)).catch(() => {});
}
